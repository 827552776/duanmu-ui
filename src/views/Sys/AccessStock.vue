...
<!--        <el-form-item>-->
<!--          <kt-button icon="fa fa-plus" :label="$t('入库录入')" perms="sys:dict:add" type="primary" @click="handleAdd" />-->
<!--        </el-form-item>-->
<!--        <el-form-item>-->
...
<!--        </el-form-item>-->
    ...
    <!--表格内容栏-->
    <PtrTable :height="500" permsEdit="sys:dict:edit" permsDelete="sys:dict:delete"
...
<script>
    ...
    import PtrTable from "../Core/PtrTable";
    export default {
        components:{
            KtTable,
            KtButton,
        ...
        methods: {
            ...
            findPage: function (data) {
                ...
                this.pageRequest.columnFilters = {name: {name:'name', value:this.filters.name},type: {name:'type', value:this.filters.type},
                  xiType: {name:'xiType', value:this.filters.xiType},qianTime: {name:'qianTime', value:this.filters.qianTime},
                  houTime: {name:'houTime', value:this.filters.houTime} }
                this.$api.access.findPage(this.pageRequest).then((res) => {
                ...
                this.$refs.dataForm.validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            this.editLoading = true
                            let params = Object.assign({}, this.dataForm)
                            this.$api.access.saveConfirm(params).then((res) => {
                                if(res.code == 200) {
                                    this.$message({ message: '操作成功', type: 'success' })
                                } else {
                                    this.$message({message: '操作失败, ' + res.msg, type: 'error'})
