...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["焊材名称", "调拨单位","来源","型号", "单位","价格", "调拨数量", "调拨出库时间","工艺备注"] // 导出的表头名
      const filterVal = ["name","deCompany", "source","model","company", "price", "deNumber", "deTime", "remarks"]
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.excelData.push(this.multipleSelection[i]);
      }
      const list = this.excelData
      const data = this.formatJson(filterVal, list)
      const filename = '焊材调拨记录' + (new Date()).toLocaleDateString();
