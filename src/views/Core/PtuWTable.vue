...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["名称", "类型","细分", "外售单位","出库时间","出库价格", "出库数量", "备注"] // 导出的表头名
      const filterVal = ["name", "type","xiType","modeBy","outTime", "price", "outNumber", "remarks"]
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.excelData.push(this.multipleSelection[i]);
      }
      const list = this.excelData
      const data = this.formatJson(filterVal, list)
      const filename = '配件外售记录' + (new Date()).toLocaleDateString();
