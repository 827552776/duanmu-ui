...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["模具名称", "数量","单位", "价格","出库时间", "入库时间",  "备注"] // 导出的表头名
      const filterVal = ["mould", "number","company","price", "startDate", "endDate", "remarks"]
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.excelData.push(this.multipleSelection[i]);
      }
      const list = this.excelData
      const data = this.formatJson(filterVal, list)
      const filename = '模具外协费用明细' + (new Date()).toLocaleDateString();
