<template>
  <div>
    <el-tooltip content="导出" placement="top">
      <el-button icon="fa fa-file-excel-o" @click="leading"/>
    </el-tooltip>
    <!--表格栏-->
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["模具名称", "材料名称","材质", "单位","派工号","批号","外协单位" ,"外协时间", "外协入库时间", "外协数量","外协价格", "工艺备注"] // 导出的表头名
      const filterVal = ["mName", "name","texture","company", "dispatch","batch", "waixie", "wxTime", "wxInt", "wxNumber","wxPrice","remarks"]
      ...
      const filename = '材料外协记录' + (new Date()).toLocaleDateString();
    ...
    // 换页刷新
      ...
      //入库
      handleEditIt:function (index,row) {
        this.$emit('handleEditIt',{index:index,row:row})
      },
      //出库
      handleEditOut:function (index,row) {
          this.$emit('handleEditOut',{index:index,row:row})
      },
    // 编辑
