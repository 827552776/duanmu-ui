...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["焊材名称", "模具名称","来源","型号", "单位","价格", "出库时间", "领用数","退回数","用量", "班组","工艺备注"] // 导出的表头名
      const filterVal = ["name","mouldName", "source","model","company", "price", "outTime", "number", "reNumber","sNumber","zu","remarks"]
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.excelData.push(this.multipleSelection[i]);
      }
      const list = this.excelData
      const data = this.formatJson(filterVal, list)
      const filename = '焊材自用记录' + (new Date()).toLocaleDateString();
