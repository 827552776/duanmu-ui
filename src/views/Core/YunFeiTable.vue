...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["时间", "地点","模具名称", "模具数量","价格", "运输人", "材料名称",  "备注"] // 导出的表头名
      const filterVal = ["aTime", "descri","mould","number", "price", "logis", "material","remarks"]
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.excelData.push(this.multipleSelection[i]);
      }
      const list = this.excelData
      const data = this.formatJson(filterVal, list)
      const filename = '运费明细（包含材料）' + (new Date()).toLocaleDateString();
