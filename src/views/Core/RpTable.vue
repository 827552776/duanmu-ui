    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column :label="$t('action.operation')" width="185" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
					<el-button  size="mini"  @click="handleEdit(scope.$index, scope.row)">报表</el-button>
					<el-button  size="mini"  @click="xiangQing(scope.$index, scope.row)">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
    ...
    // 报表
      ...
      this.$emit('handleEdit', {index:index, row:row})
		...
		xiangQing: function (index, row) {
		  this.$emit('xiangQing', {index:index, row:row})
		},
