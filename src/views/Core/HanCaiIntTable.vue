...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      const header = ["焊材名称", "来源","型号", "单位","价格", "入库时间", "入库数量", "工艺备注"] // 导出的表头名
      const filterVal = ["name", "source","model","company", "price", "intTime", "number", "remarks"]
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.excelData.push(this.multipleSelection[i]);
      }
      const list = this.excelData
      const data = this.formatJson(filterVal, list)
      const filename = '焊材入库记录' + (new Date()).toLocaleDateString();
